import { useEffect, useState } from "react";
import { auth } from "../utils/FirebaseConfig";
import { uploadImageProfile } from "../services/imageService";
import {
  getUserDataFromDatabase,
  updateUserDatabaseField,
} from "../services/userService";
import { useImagePicker } from "./useImagePicker";
import { ActivityIndicator, View, Text } from "react-native";

const dummyProfileData = {
  imageUrl:
    "data:image/jpeg;base64,",
  username: "ERROR_USER",
  placed: false,
  phoneNumber: "NOT SET",
  email: "error@error.error",
  realname: "NOT SET",
  collective: "NOT SET",
  room: "NOT SET",
};

export const useProfileSettings = () => {
  const [userData, setUserData] = useState<any>(undefined);
  const [activeField, setActiveField] = useState<string | null>(null);
  const [tempValue, setTempValue] = useState("");
  const [loadingImage, setLoadingImage] = useState(false);

  const gallery = useImagePicker();

  useEffect(() => {
    const fetchData = async () => {
      const data = await getUserDataFromDatabase();
      setUserData(data);
    };
    fetchData();
  }, []);

  const handleStartEdit = (fieldKey: string, currentValue: string) => {
    setActiveField(fieldKey);
    setTempValue(currentValue);
  };

  const handleSave = async (fieldKey: string) => {
    const success = await updateUserDatabaseField(fieldKey, tempValue);
    if (success) {
      setUserData({ ...userData, [fieldKey]: tempValue });
      setActiveField(null);
    } else {
      alert("Save failed. Please try again.");
    }
  };

  const handleUpdateProfileImage = async () => {
    const imageUri = await gallery.pickImage();

    if (imageUri) {
      setLoadingImage(true);
      const userId = auth.currentUser?.uid;

      if (!userId) {
        setLoadingImage(false);
        return;
      }

      try {
        const downloadUrl = await uploadImageProfile(userId, imageUri);
        await updateUserDatabaseField("imageUrl", downloadUrl);
        setUserData({ ...userData, imageUrl: downloadUrl });
        alert("Profilbilde oppdatert!");
      } catch (error) {
        console.error(error);
        alert("Feil ved opplasting");
      } finally {
        setLoadingImage(false);
        gallery.resetPicker();
      }
    }
  };

  return {
    username: userData?.username || dummyProfileData.username,
    email: userData?.email || dummyProfileData.email,
    phoneNumber: userData?.phoneNumber || dummyProfileData.phoneNumber,
    imageUrl: userData?.imageUrl || dummyProfileData.imageUrl,
    placed: userData?.placed || dummyProfileData.placed,
    realname: userData?.realname || dummyProfileData.realname,
    collective: userData?.collective || dummyProfileData.collective,
    room: userData?.room || dummyProfileData.room,

    activeField,
    tempValue,
    loadingImage,
    setTempValue,
    handleStartEdit,
    handleSave,
    handleUpdateProfileImage,
  };
};
